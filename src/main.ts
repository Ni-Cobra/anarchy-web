...
function runMain(): void {
  ...
  // cell empty, target in reach, no player circle overlapping the cell. The
  // ghost preview and the right-click handler both gate on this so the UI
  // never invites a click the server will silently drop. Strict-less
  // circle-vs-cell overlap matches the server's check, including the
  // "tangent against the cell" exception.
  function canPlaceAt(
    cx: number,
    cy: number,
    lx: number,
